...
const renderSections = () => {
  ...
  state.sections.forEach((section, sectionIndex) => {
    ...
    const addQuestionBtn = card.querySelector(
      `[data-add-question="${section.id}"]`
    );
    addQuestionBtn.addEventListener('click', () => {
      section.questions.push({
        id: `${section.id}-q${section.questions.length + 1}`,
        text: '',
        type: 'text',
        weight: 1
      });
    ...
    section.questions.forEach((question, questionIndex) => {
      ...
      const weightField = wrapper.querySelector(
        `#question-weight-${question.id}`
      );
      const removeQuestion = wrapper.querySelector(
        `[data-remove-question="${section.id}:${question.id}"]`
      );
...
const addSection = (defaults = {}) => {
  ...
  state.sections.push({
    ...
    questions: defaults.questions?.map((question, idx) => ({
    ...
    standard: [
      {
        ...
        questions: [
          {
            text: 'Summarise business objectives and success criteria.',
            type: 'text',
            weight: 2
          },
          {
            text: 'List current challenges or constraints.',
            type: 'text',
            weight: 1
          }
        ]
      },
      {
        ...
        questions: [
          {
            text: 'Describe integration points and data sources.',
            type: 'multi-select',
            weight: 2
          },
    ...
    security: [
      {
        ...
        questions: [
          {
            text: 'Outline certifications and attestations held.',
            type: 'multi-select',
            weight: 2
          },
          {
            text: 'Describe data protection and privacy controls.',
            type: 'text',
            weight: 2
          }
        ]
    ...
    integration: [
      {
        ...
        questions: [
          ...
          {
            text: 'List supported authentication mechanisms.',
            type: 'multi-select',
            weight: 1
          }
        ]
...
generateButton?.addEventListener('click', async () => {
  ...
  try {
    ...
    renderSections();
    document.getElementById('rfx-title').value =
      template.title || 'Generated RFX';
  } catch (err) {
